        ...
        try:
            ...
            try:
                ...
                if actual_displayed_date_str:
                    parsed_displayed_date = datetime.strptime(actual_displayed_date_str, "%d/%m/%Y")
...
if __name__ == "__main__":
    ...
    if not scraper.redis_client.exists(INITIAL_POP_MARKER_KEY) or \
       (len(scraper.redis_client.keys(f"{datetime.today().year}-*")) < DAYS_FOR_INITIAL_POPULATION / 2) : # Heuristic
